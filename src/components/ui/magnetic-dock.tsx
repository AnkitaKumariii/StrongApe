    ...
    return (
        <motion.button
            ...
            className={cn(
                ...
                "focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 dark:focus-visible:ring-white/50",
                item.isActive && "bg-primary/10 dark:bg-primary/20"
            )}
            style={{
                width: size,
                height: size,
        ...
        >
            ...
            >
                ...
                <div className={cn(
                    ...
                    item.isActive ? "text-primary dark:text-primary" : "text-slate-600 dark:text-slate-300"
                )}>
                    {item.icon}
                </div>
            ...
            <AnimatePresence>
                {showLabels && isHovered && (
                    <motion.div
                        initial={{ opacity: 0, y: isVertical ? 0 : 8, x: isVertical ? 8 : 0, scale: 0.9 }}
                        animate={{ opacity: 1, y: 0, x: 0, scale: 1 }}
                        exit={{ opacity: 0, y: isVertical ? 0 : 8, x: isVertical ? 8 : 0, scale: 0.9 }}
                        transition={{ duration: 0.15, ease: "easeOut" }}
                        className={cn(
                            "absolute",
                            isVertical ? "left-full top-1/2 -translate-y-1/2 ml-4" : "-top-10 left-1/2 -translate-x-1/2",
                            "px-3 py-1.5 rounded-lg",
                            "bg-white dark:bg-slate-900/95",
                            "backdrop-blur-sm",
                            "text-slate-800 dark:text-white text-sm font-medium whitespace-nowrap",
                    ...
                    >
                        ...
                        <div
                            className={cn(
                                "absolute",
                                isVertical 
                                    ? "top-1/2 -translate-y-1/2 -left-1 border-l border-b border-slate-200 dark:border-white/10" 
                                    : "left-1/2 -translate-x-1/2 -bottom-1 border-r border-b border-slate-200 dark:border-white/10",
                                "w-2 h-2 rotate-45",
                                "bg-white dark:bg-slate-900/95"
                            )}
                        />
...
function MagneticDock({
    ...
    return (
        <motion.div
            ...
            className={cn(
                "inline-flex gap-2 p-3 rounded-3xl",
                isVertical ? "items-start" : "items-end",
        ...
        >
            {items.map((item) => (
                <div key={item.id} className={cn("flex items-center", isVertical ? "flex-row" : "flex-col")}>
                    <DockItem
                        item={item}
                        ...
                        showLabels={showLabels && !isExpanded}
                    ...
                    <AnimatePresence>
                        {isExpanded && isVertical && (
                            <motion.div
                                ...
                                className="overflow-hidden whitespace-nowrap font-medium text-slate-800 dark:text-slate-200"
                            >
                                {item.label}
                            </motion.div>
                        )}
...
// Export specific type and components
export {
    MagneticDock,
    DockIconHome,
    DockIconSearch,
